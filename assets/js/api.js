...
class SkinIAAPI {
  constructor() {
    // Don't cache baseURL - always read from CONFIG dynamically
    this.apiKey = null;
  }

  // Get base URL dynamically from localStorage
  get baseURL() {
  ...
  async analyzeImage(file, framework = "pytorch") {
    try {
      ...
      const endpoint =
        framework === "pytorch" ? "/v1/analyze-pytorch" : "/v1/analyze";
    ...
    return new Promise((resolve, reject) => {
      const interval = setInterval(async () => {
        ...
        try {
          ...
          consecutiveErrors++;
          console.warn(
            `Status check attempt ${attempts} failed (${consecutiveErrors} consecutive errors):`,
            error.message
          );
  ...
  async triggerElection() {
    try {
      const response = await fetch(
        `${this.baseURL}/v1/distributed/trigger-election`,
        {
          method: "POST",
          headers: this.getHeaders(),
        }
